...
## Start edit 2025 Oct 16 a, 2025 Oct 27 a
...
names(myColors.FamGen) <- 
  sort(c(levels(
    factor(c((PollenIDs %>% filter(.data$Genus %in% topGenera2$Genus) %>% mutate(FamGen=paste(Family, Genus, sep=" ")) %>% arrange(Family) %>% select(FamGen))$FamGen,"z_AllOther", "Fabaceae Robinia"), 
    as.character(unique(c((PollenIDs %>% filter(.data$Genus %in% topGenera2$Genus) %>% mutate(FamGen=paste(Family, Genus, sep=" ")) %>% arrange(Family) %>% select(FamGen))$FamGen,"z_AllOther", "Fabaceae Robinia"))))
    ), "z_Shared"))
...
dat2<-as.matrix(  dat %>% select(-c(Barcode:NLCD1000_Crop, 'NA', any_of(singletonGenera)))  )
rownames(dat2)<-dat$BeeID
sort(colnames(dat2))
...
#Try removing samples appearing as outliers in the intital NMDS with raw PropReads
...
dat %>%
  ...
  ggplot(aes(x=Bombus_spp)) +
  geom_col(aes(y=PropReads, 
               fill=ifelse(.data$Genus %in% c(topGenera2$Genus, "Robinia"), FamGen, 
                       ifelse(!is.na(.data$Genus), "z_AllOther", NA))), 
              position="fill",
           color="white") +
  scale_fill_manual(values = myColors.FamGen, na.value="gray")+ #FamGen.palette
  ...
  facet_wrap(~Barcode, ncol=1, scales="free_y")+
  theme_bw(base_size = 16)+
  labs(title="", y="Proportion of Reads", x="Bombus species")+
  theme(legend.text = element_text(size=10),
        legend.position = "bottom")
## END edit
...
dat<- left_join(
  ...
  mutate(CapTimeHM=hms::hms(hours=CapTimeH, minutes=CapTimeM, seconds=0), JDate=julian(Date)) %>%  #create variables: CapTimeHM and julian date JDate
  mutate(HostGenus=ifelse(is.na(.data$HostGenus),"none",HostGenus)) %>%
  mutate(max=max(c_across(Prunus:Phleum))) %>% 
  filter(Bombus_spp=="bimaculatus" | Bombus_spp=="griseocollis"| Bombus_spp=="impatiens") %>%
#  filter(Name=="Blandy Experimental Farm") %>%
#  filter(BeeID %in% TwoBarcodeBees$BeeID) %>%
  ungroup()
...
dat2<-vegan::decostand(dat2, method = "hellinger") # community data standardization for raw PropReads
...
RDA_Bombus<-vegan::rda(dat2 ~ HostGenus + Barcode + JDate + Bombus_spp + Updated_y, data=dat) 
...
sort(species.long2$diff)

arrows <- as.data.frame(vegan::scores(RDA_Bombus)$biplot) #
arrows$names<-rownames(arrows)

arrows <- arrows %>%        # select only Host and Barcode arrows greater than 0.15 units from 0,0
  mutate(diff=sqrt(abs(RDA1)^2+abs(RDA2)^2)) %>% 
  filter(
    !str_starts(names, "Bombus"), #Bombus will be visualized with color
    !str_starts(names,"Barcode"), #Barcode will be visualized with shape (or not at all)
    diff>0.3
         ) %>%
  rowwise() %>% mutate(
...
set.seed(73); ggplot() + 
    ...
    geom_point(data=sites.long, 
               aes(x=axis1, y=axis2, color=Bombus_spp, shape=Bombus_spp), 
               size=5, alpha=0.7) +
    scale_color_manual(values=sample(colorBlindness::SteppedSequential5Steps, size = 3, replace= FALSE)) +
#    stat_ellipse(data=sites.long, aes(x=axis1, y=axis2, colour=Bombus_spp), linewidth = 1) +
    ...
    ggrepel::geom_text_repel(data=rbind(species.long2, arrows), aes(x = axis1, y = axis2, label = labels), direction = "both",  box.padding = 0.75, nudge_x = c(rep(0, 10), +0.25, rep(0,5))) +
    geom_segment(data=arrows, aes(x = 0, xend = axis1, y = 0, yend = axis2), arrow = arrow(length = unit(0.25, "cm")), colour="black") +
    coord_fixed(ratio=1) +
    theme_bw(base_size = 12)+
    labs(title=paste("CLR-prop \nBombus species differences (n = ",records," records from ",bees," bees)", sep=""))+
...
arrows$names<-rownames(arrows)
arrows <- arrows %>%        # select non-County arrows greater than x.xx units from 0,0
  mutate(diff=sqrt(abs(RDA1)^2+abs(RDA2)^2)) %>% 
