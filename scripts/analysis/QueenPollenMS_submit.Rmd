...
# creating a new SampleTotalReads that only includes ASVs assigned to Genus 
...
PollenIDgenera.sum1 <- PollenIDs %>%
  group_by(Barcode, BeeID, Genus) %>% #group by barcode, sample/BeeID, and assigned genus
  summarize(PropAssgnReads=sum(PropAssgnReads)) %>% # sum the proportion of reads 
  pivot_wider(., names_from = "Genus", values_from = "PropAssgnReads") %>% # begin transformation by pivoting the table wider into community dataset
  mutate(across(everything(), ~replace(., is.na(.), 0))) %>% # replace NA with 0
  rowwise() %>% mutate(rareGenera=sum(c_across(any_of(singletonGenera)))) %>% # create new column (taxa) called rareGenera to be sum of PropAssgnReads belonging to singletonGenera
  rename(., unassigned = "NA") %>% #rename cumbersome column name "NA"
  select(-c(any_of(singletonGenera), unassigned)) #exclude singletonGenera (taxa appearing in one sample only) and exclude unassigned

rowSums(PollenIDgenera.sum1[,-c(1:2)]) #row sums to 1 for every sample; the sum of Proportion of Assigned Reads belonging to each Genera and rareGenera equals 1 within each sample (reads un-assigned to genus have been removed; earlier, see "PollenIDs", the proportion of reads assigned to taxa were calculated relative to total assigned reads in a sample, rather than sample total reads incl assigned and not)

detect.threshold <- min(PollenIDs$Reads) / max(PollenIDs$SampleAssgnReads) #4.5e-05

dat.temp <- compositions::rcomp(PollenIDgenera.sum1 #invoke rcomp() : A class providing a way to analyse compositions in the philosophical framework of the Simplex as subset of the R^D
                                %>% mutate(rareGenera=rareGenera+4.6e-5), #add constant to rareGenera that is just above the detection threshold
...
clr<-compositions::clr(dat.temp)
summary(dat.temp)

rowSums(as_tibble(clr)) # CLR does not play nice with "pure" samples containing data from only one taxa/genus.
as_tibble(clr)

PollenIDgenera.sum1$rareGenera
as_tibble(clr)$rareGenera
as_tibble(clr)$Prunus

# NEXT STEP = PIVOT LONGER TO MAKE IT ALL LOOK LIKE POLLENIDGENERA.SUM
...
biplot(pcaPP::PCAproj(dat.temp, k=3, scores=TRUE), main="raw data rPCA \n w/ package:pcaPP")

colnames(dat.temp)
NMDS = vegan::metaMDS(as.matrix(compositions::clr(dat.temp)), k = 2, trymax = 20) #all data
NMDS
stressplot(NMDS)
