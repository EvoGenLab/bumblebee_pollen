...
NA.BeeIDs<-PollenIDs %>% # identifying bees with more than half of sample reads unassigned
  filter(is.na(Genus),PropReads>0.5) %>%
  select(BeeID)

PollenIDs <- PollenIDs %>%
  filter(!.data$BeeID %in% NA.BeeIDs$BeeID) # removing bees with more than half of sample reads unassigned
...
## PollenIDs.sum & PollenIDgenera.sum
```{r}
...
### Center log-transformation Transform 
```{r}
...
# (a) the resulting parts are still linear dependent:  The first property yields singular covariance matrices, i.e. the determinant zero, which becomes problematic if a statistical method applied needs to invert the covariance matrix (e.g. PCA, LDA, etc.).
# (b) the transformation is sub-compositionally incoherent: The latter property means that when different subsets of variables (parts) are considered, the clr-transformed results are not the same. This has serious consequences for the data analysis, because any chosen bivariate subset of interest would not reflect the original data. 

# (we always remove NA and singletonGenera from NMDS and RDA ordination analyses - so we're not comparing across different subsets)
...
#  select(-c(any_of(singletonGenera))) # (optional) we will exclude singletonGenera and NA from the NMDS and RDA analyses, but we're including them here so that the samples each sum to 1.0 for PropReads
  as_tibble(.)
...
# view PCA with raw proportions (before transformation)
biplot(pcaPP::PCAproj(PollenIDgenera.sum1[,-c(1:2)], k=2, scores=TRUE), main="raw PropReads data rPCA \n w/ package:pcaPP")

# calculate detection threshold for use in compositions::rcomp()
detect.threshold <- min(PollenIDs$Reads) / max(PollenIDs$SampleTotalReads) #4.5e-05

#invoke rcomp() : A class providing a way to analyse compositions in the philosophical framework of the Simplex as subset of the R^D
rcomp_PollenIDgenera.sum1 <- compositions::rcomp(PollenIDgenera.sum1, 
                                parts=3:ncol(PollenIDgenera.sum1),
                                total=1,
                                detectionlimit = detect.threshold,
                                BDL=NULL,MAR=NULL,MNAR=NULL,SZ=NULL)

#summary(rcomp_PollenIDgenera.sum1)
...
# perform the transformation, coerce to tibble
...
# view PCA with CLR-proportions (after transformation)
...
```{r}
#how many samples (bees)?
PollenIDs %>% summarise(n=n_distinct(BeeID))

# count of ITS2, rbcL, both samples by Bombus species
dat<- left_join( #this dataset has Bombus_spp identity and keeps data from the two barcodes separate
...
dim(PollenIDs %>% 
  ...
  summarize(
    n=n()
  )) # three columns (BeeID, Barcode, n or number of records)
...
ggplot(aes(x = rank,
  ...
  facet_wrap(~Barcode) +
  labs(x="Rank", y="Proportion of Reads") +
  theme_bw(base_size=18) +
  theme(legend.position = "none", 
...
temp %>% filter(metric=="Reads") %>% mutate(topGeneraTotalReads=sum(value)) %>% select(topGeneraTotalReads) %>% distinct() # 926042 reads assigned to top genera
926042/sum(PollenIDs$Reads) #~79% of ALL reads belong to one of the top genera
PollenIDs %>% filter(is.na(Genus)) %>% mutate(NAReads=sum(Reads)) %>% select(NAReads) %>% distinct() #56373 unassigned (NA) reads
926042/(sum(PollenIDs$Reads)-56373) #~83% of assigned reads belong to one of the top genera

topGenera2<-temp$Genus %>% as_tibble() %>% distinct() %>% as.data.frame()
colnames(topGenera2)<-"Genus"

# making a pretty figure to show the top genera and their 'abundance' metrics
temp %>%
  ggplot() + 
...
myColors.FamGen <- c("#990F0F", "#B22C2C" , "#CC5151" , "#E57E7E" , "#FFB2B2" , "#99540F" , "#B26F2C" , "#CC8E51" , "#E5B17E" , "#FFD8B2" , "#6B990F" , "#85B22C" , "#A3CC51" , "#C3E57E" , "#E5FFB2" , "#0F6B99" , "#2C85B2" , "#51A3CC" , "#7EC3E5" , "#B2E5FF" , "#260F99" , "#8F7EE5", "#BFB2FF", "black")
...
```{r}
# how many species per pollen genera are in the dataset
head(PollenIDs %>% 
...
# on average, many species per pollen genera are in the dataset
PollenIDs %>% 
       filter(Genus %in% topGenera2$Genus) %>% 
       filter(!is.na(Species)) %>%
       group_by(Genus, Barcode) %>% 
...
CapLog %>% filter(HostGenus!="") %>% summarize(bees=n_distinct(BeeID)) #414 total foraging bees
...
dat.Queen %>% filter(HostGenus!="") %>% summarize(bees=n_distinct(BeeID)) #98 total foraging bees with pollen data
dat.Queen %>% 
  filter(HostGenus!="") %>% 
  group_by(HostGenus) %>% 
  summarize(bees=n_distinct(BeeID), propbees=n_distinct(BeeID)/98) %>% #98 total foraging bees with pollen data
  arrange(-bees) %>% 
  mutate(cumsum=cumsum(propbees)) 
```
...
dat<-PollenIDs %>% left_join(., dat.Queen %>% select(BeeID, SiteID, Bombus_spp, Date), by="BeeID") %>%
  ...
  summarise(weekly_value=sum(PropReads)) %>% 
  group_by(Week, Barcode) %>% mutate(weekly_total=sum(weekly_value), weekly_prop=weekly_value/weekly_total) %>% 
  mutate(label_if=ifelse(!.data$Genus %in% topGenera2$Genus & 
                             .data$weekly_prop>0.20 & 
                             !is.na(.data$Genus), FamGen, NA)) %>%
  ungroup() 

dat %>% 
  ggplot()+ 
  geom_col(aes(x=Week, y=weekly_prop,
  ...
  geom_label(data=dat%>%filter(!is.na(label_if)), 
            aes(x=Week, y=weekly_prop, label=paste(Family, Genus, sep="\n"), fontface = "italic"),
            position = "identity", vjust=c(2, 1, -1.5, 5, 2), #hjust=0.2,
            size=3 #, angle=80
           ) +
...
dat<- left_join(PollenIDgenera.sum %>% pivot_wider(., names_from = "Genus", values_from = "PropReads"), 
  ...
  select(Barcode, BeeID, SiteID, Name, JDate,
         Prunus:Phleum, 
         -"NA") %>%
...
dat %>%
  ggplot(aes(x=JDate, y=ShannonD, color=Name))+
  geom_point(size=4)+
...
# Does pollen composition differ between ITS2 and rbcL? (ALL DATA)
...
attr(venn_assigns, "intersections")[["A"]] #21 genera detected only in ITS2 samples
attr(venn_assigns, "intersections")[["A:B"]] #29 genera detected by both
attr(venn_assigns, "intersections")[["B"]] #44 genera detected only in rbcL samples
...
print(n.elements)
#from lines starting 'list<-list()' ...to... 'print(n.elements)': this code was largely copied from https://stackoverflow.com/questions/23559371/get-the-list-of-items-in-venn-diagram
...
## Genera richness, Shannon diversity ~ barcode
### Richness
```{r}
dat<-PollenIDs %>%
  group_by(BeeID,Barcode)%>%
...
hist(dat$GeneraRichness); stats::shapiro.test(dat$GeneraRichness) # not normal
#summary(stats::aov(GeneraRichness ~ Barcode, data = dat)) # so this test not valid
stats::kruskal.test(GeneraRichness ~ Barcode, data = dat) # sig difference in richness detected with rbcL versus ITS2
...
dat2<-as.matrix(  dat %>% select(-c(Barcode:NLCD1000_Crop, 'NA', any_of(singletonGenera)))  ) # select just the metabarcoding data, sans rare genera
rownames(dat2)<-dat$BeeID
sort(colnames(dat2))
...
(length(dat)) # sample size ( n = barcode-samples )

# plot shannon diversity of ITS2 and rbcL samples
ggplot(dat, aes(x = Barcode, y = ShannonD, fill = Barcode)) +
  geom_boxplot() +
#  scale_fill_manual(values = colorBlindness::SteppedSequential5Steps) +
  geom_label(data=dat  %>% group_by(Barcode) %>% 
    summarize(n = n()) %>% mutate(label = "n") %>% 
    unite("n_label", label, n, sep = " = ", remove = FALSE),
  aes(x = Barcode, y = -0.1, label = n_label), fill="white") +
  labs(x = "Barcode",
       y = "Shannon diversity")+
  theme_bw(base_size = 18)+
  theme(legend.position = "none",
...
#----- compare NMDS of barcode-samples with CLR-transformed data
#this dataset is dat.Queens plus dat.Sites AND it also keeps data from the two barcodes separate (CLR TRANSFORMED)
dat<- left_join(
  PollenIDgenera.sum1 %>% pivot_wider(., names_from = "Genus", values_from = "CLR_PropReads"), #CLR-transformed data!!!!
  dat.Queen %>% select(Date:NestSeek), by="BeeID") %>% 
  left_join(., dat.Sites, by=c("Date", "SiteID")) %>% 
  mutate(across(Prunus:Phleum, ~replace(., is.na(.), 0))) %>%
  select(Barcode, BeeID, Date, Bombus_spp, SiteID, Name, Updated_x, Updated_y, HostGenus, CapTimeH, CapTimeM, NLCD1000_Water:NLCD1000_Crop, Prunus:Phleum) #%>% filter(BeeID %in% TwoBarcodeBees$BeeID)

# select just metabarcoding data
colnames(dat)
...
dat2mat<-as.matrix(dat2) # community matrix

dat2mat.dist<-vegan::vegdist(dat2mat, method="euclidean") # aitchison_dist is same as Euclidean in CLR space
NMDS_barcode<-vegan::metaMDS(dat2mat.dist)

vegan::stressplot(NMDS_barcode)
# vegan::goodness(NMDS_barcode)
# ordiplot(NMDS_barcode, type="text", cex=1.2)

#removing poorly fit from previous - same qualitative result with slightly lower stress (0.25)
# subset<-as.vector(which(vegan::goodness(NMDS_barcode)>0.03))
# NMDS_barcode2 = vegan::metaMDS(vegan::vegdist(dat2mat[-c(subset),], method="euclidean"), k = 2, trymax = 20)
# stressplot(NMDS_barcode2)
# NMDS_barcode2
# NMDS_plot_df<- cbind(dat[-c(subset),] %>% ungroup(),
#                  vegan::scores(NMDS_barcode2, display = "sites") %>%
#   as.data.frame())
# ggplot(NMDS_plot_df, aes(x=NMDS1, y=NMDS2, color=Barcode, shape = Barcode))+ # plot NMDS by Barcode
#   geom_point(size = 4, alpha=0.5) +
#   stat_ellipse(linetype = 2, linewidth = 1) +
#   labs(title = "CLR-transformed proportions") +
#   theme_bw(base_size = 18)


# save sample size and stress for optional plotting below
(stress<-round(NMDS_barcode$stress, digits=2))
(bees<-n_distinct(dat$BeeID))
(records<-length(dat$BeeID))

# making a nice plot of the NMDS ordination 
NMDS_plot_df<- cbind(
  dat %>% ungroup(), # community dataset
  vegan::scores(NMDS_barcode, display = "sites") %>% as.data.frame()) # site scores
NMDS_plot_df_2<-cbind( 
  vegan::envfit(NMDS_barcode, dat2mat)$vectors$arrows %>% as.data.frame(), #species scores
  vegan::envfit(NMDS_barcode, dat2mat)$vectors$pvals %>% as.data.frame(), #pvalues
  vegan::envfit(NMDS_barcode, dat2mat)$vectors$r %>% as.data.frame()) #R2
NMDS_plot_df_2$Genus<-rownames(NMDS_plot_df_2); colnames(NMDS_plot_df_2)<-c("NMDS1", "NMDS2", "p.value", "r2", "Genus")
NMDS_plot_df_2<-NMDS_plot_df_2 %>% mutate(NMDS1=NMDS1*sqrt(r2)*5, NMDS2=NMDS2*sqrt(r2)*5) # scale arrow length by r2 value and multiply by a constant for easier viewing
ggplot()+ 
  geom_point( # plot species 
    data = NMDS_plot_df_2,
    aes(x = NMDS1, y = NMDS2),
    size=2, shape=1) +
  geom_point( # plot sites (points) color/shape by Barcode
    data=NMDS_plot_df, 
    aes(x=NMDS1, y=NMDS2, color=Barcode, shape=Barcode), 
    size=4, alpha=0.8) +
  ggrepel::geom_text_repel(
      data = NMDS_plot_df_2 %>% filter(Genus %in% topGenera2$Genus), #filter(r2>0.15), #optional filter
      aes(x = NMDS1, y = NMDS2, label = Genus, fontface = "italic"), 
      direction = "both",  box.padding = 0.75, size=5, max.overlaps = 10
      ) + #, nudge_x = c(rep(0, 10), +0.25, rep(0,5))  
  stat_ellipse( # circle groups
    data=NMDS_plot_df, 
    aes(x=NMDS1, y=NMDS2, color=Barcode), 
    linetype = 2, linewidth = 1) +
#  labs(title=paste("NMDS / stress = ",stress, "\n(n = ",records," records from ",bees," bees)", sep=""))+ #optional
  coord_fixed()+
  theme_bw(base_size = 18) +
  theme(legend.text = element_text(face = "italic"))

# hmm plotting blandy samples colored by date just for fun
# ggplot(NMDS_plot_df %>% filter(str_starts(SiteID,"BEF")), 
#        aes(x=NMDS1, y=NMDS2, color=Date, shape = Barcode))+ # plot NMDS by date at BLANDY
#   geom_point(size = 4) +
#   labs(title = "CLR-transformed proportions") +
#   theme_bw(base_size = 18)


# dispersion testing and PERMANOVA
dispersion <- vegan::betadisper(dat2mat.dist, group=dat$Barcode)
TukeyHSD(dispersion)
vegan::permutest(dispersion)
plot(dispersion, hull=FALSE, ellipse=TRUE) ##sd ellipse
vegan::adonis2(dat2~ Barcode, data=dat, permutations = 999, method = "euclidean", by="term")

#----- compare NMDS of barcode-samples with raw proportions data
#this dataset is dat.Queens plus dat.Sites AND it also keeps data from the two barcodes separate (RAW PROPORTIONS)
dat<- left_join(
  PollenIDgenera.sum %>% pivot_wider(., names_from = "Genus", values_from = "PropReads"), #raw PropReads
  dat.Queen %>% select(Date:NestSeek), by="BeeID") %>% 
  left_join(., dat.Sites, by=c("Date", "SiteID")) %>% 
  mutate(across(Prunus:Phleum, ~replace(., is.na(.), 0))) %>%
...
dat2mat.dist<-vegan::vegdist(dat2mat, method="bray") # Bray-Curtis distance
NMDS_barcode<-vegan::metaMDS(dat2mat.dist)
...
#----- compare PCA with raw and clr-transformed data
biplot(
  pcaPP::PCAproj(
    PollenIDgenera.sum %>% 
      pivot_wider(., names_from = "Genus", values_from = "PropReads", values_fill = 0) %>% ungroup() %>%
      select(Prunus:Phleum) %>% 
      select(-c(any_of(singletonGenera), "NA")),
    k=2, scores=TRUE), 
  main="raw data rPCA \n w/ package:pcaPP")
biplot(
  pcaPP::PCAproj(
    PollenIDgenera.sum1 %>% 
      pivot_wider(., names_from = "Genus", values_from = "CLR_PropReads", values_fill = 0) %>% ungroup() %>%
      select(Prunus:Phleum) %>% 
      select(-c(any_of(singletonGenera), "NA")), 
    k=2, scores=TRUE), 
  main="CLR-transformed data rPCA \n w/ package:pcaPP")
...
And all three NMDS of raw proportions show rbcL clustering a little apart from ITS2, but with the CLR-transformed proportions they overlap completely


# Does pollen composition vary among Bombus species? (YES Blandy, NO Rare bees)

## NEW: NMDS of Bombus_spp samples
```{r}
#----- compare NMDS of barcode-samples with CLR-transformed data
#this dataset is dat.Queens plus dat.Sites AND it also keeps data from the two barcodes separate (CLR TRANSFORMED)
dat<- left_join(
...
dat2mat.dist<-vegan::vegdist(dat2mat, method="euclidean") # aitchison_dist is same as Euclidean in CLR space
NMDS_Bombus_spp<-vegan::metaMDS(dat2mat.dist)   

vegan::stressplot(NMDS_Bombus_spp)
#vegan::goodness(NMDS_Bombus_spp)
#vegan::ordiplot(NMDS_Bombus_spp, type="text", cex=1.2)


(stress<-round(NMDS_Bombus_spp$stress, digits=2))
(bees<-n_distinct(dat$BeeID))
(records<-length(dat$BeeID))
...
NMDS_plot_df<- cbind(
  dat %>% ungroup(), # community dataset
  vegan::scores(NMDS_Bombus_spp, display = "sites") %>% as.data.frame()) # site scores
NMDS_plot_df_2<-cbind( 
  vegan::envfit(NMDS_Bombus_spp, dat2mat)$vectors$arrows %>% as.data.frame(), #species scores
  vegan::envfit(NMDS_Bombus_spp, dat2mat)$vectors$pvals %>% as.data.frame(), #pvalues
  vegan::envfit(NMDS_Bombus_spp, dat2mat)$vectors$r %>% as.data.frame()) #R2
NMDS_plot_df_2$Genus<-rownames(NMDS_plot_df_2); colnames(NMDS_plot_df_2)<-c("NMDS1", "NMDS2", "p.value", "r2", "Genus")
NMDS_plot_df_2<-NMDS_plot_df_2 %>% mutate(NMDS1=NMDS1*sqrt(r2)*5, NMDS2=NMDS2*sqrt(r2)*5) # scale arrow length by r2 value and multiply by a constant for easier viewing
ggplot()+ 
  geom_point( # plot species 
    ...
    aes(x = NMDS1, y = NMDS2),
    size=2, shape=1) +
  geom_point( # plot sites (points) color/shape by Bombus spp
    data=NMDS_plot_df, 
    aes(x=NMDS1, y=NMDS2, color=Bombus_spp, shape=Bombus_spp), 
    size=4, alpha=0.7) +
  ggrepel::geom_text_repel(
      data = NMDS_plot_df_2 %>% filter(Genus %in% topGenera2$Genus), #filter(r2>0.15),
      aes(x = NMDS1, y = NMDS2, label = Genus, fontface = "italic"), 
      direction = "both",  box.padding = 0.75, size=5, max.overlaps = 10
      ) + #, nudge_x = c(rep(0, 10), +0.25, rep(0,5))  
  stat_ellipse( # circle groups
    data=NMDS_plot_df, 
    aes(x=NMDS1, y=NMDS2, color=Bombus_spp), 
    linetype = 2, linewidth = 1) +
#  labs(title=paste("NMDS / stress = ",stress, "\n(n = ",records," records from ",bees," bees)", sep=""))+
  coord_fixed()+
  theme_bw(base_size = 18)+
  theme(legend.text = element_text(face = "italic"),
        legend.title = element_text(face = "italic"))

dispersion <- vegan::betadisper(dat2mat.dist, group=dat$Bombus_spp)
TukeyHSD(dispersion)
vegan::permutest(dispersion)
plot(dispersion, hull=FALSE, ellipse=TRUE) ##sd ellipse
vegan::adonis2(dat2~ Bombus_spp, data=dat, permutations = 999, method = "euclidean", by="term")
...
dat2<-dat %>% # dat2 is basically same as dat, but...
  ...
  group_by(Bombus_spp, Barcode) %>% mutate(spp_total=sum(PropReads), spp_prop=PropReads/spp_total) %>% 
  mutate(label_if=ifelse(!.data$Genus %in% topGenera2$Genus & 
                             .data$spp_prop>0.20 & 
                             !is.na(.data$Genus), FamGen, NA))

# making a pretty plot of pollen composition (prop of reads) from each species
dat2 %>%
  ...
  geom_label(data=dat2%>%filter(!is.na(label_if)), 
            aes(x=Bombus_spp, y=spp_prop, label=paste(Family, Genus, sep="\n"), fontface = "italic"),
            position = "identity", vjust=c(2, 0.9, 3, 2.5, 1, 3), #hjust=0.2,
            size=3 #, angle=80
           ) +
  geom_label(data=dat %>% # adding sample size to plot based on unique BeeID in dat
               group_by(Bombus_spp) %>%
               summarize(num=n_distinct(BeeID), Barcode="rbcL"),
             aes(x=Bombus_spp, y=-0.05, label=paste("n = ", num, sep="")), size=4)+
    guides(fill=guide_legend(title = "Family Genus"))+
  facet_wrap(~Barcode, ncol=1, scales="free_y")+
  labs(title="", y="Proportion of Reads", x="Bombus spp")+
  theme_bw(base_size = 18)+
  theme(legend.text = element_text(size=10, face = "italic"),
        legend.position = "bottom",
        strip.text = element_text(face = "italic"),
...
dat2 %>% group_by(Bombus_spp, Barcode) %>% summarise(total=sum(PropReads))
...
## Genera richness, Shannon diversity ~ Bombus_spp
### Genera richness
```{r}
dat<-PollenIDs %>%
...
summary(stats::aov(GeneraRichness ~ Bombus_spp, data = dat)) # no sig difference in richness between most common species, *****unless you exclude bees without data from BOTH barcodes
...
```{r}
dat<- left_join(PollenIDgenera.sum %>% pivot_wider(., names_from = "Genus", values_from = "PropReads"), 
  dat.Queen %>% select(Date:NestSeek), by="BeeID") %>% ungroup() %>%
  arrange(BeeID) %>% 
#  filter(.data$BeeID %in% TwoBarcodeBees$BeeID) %>% #optional filter for BOTH BARCODE BEES
  mutate(across(Prunus:Phleum, ~replace(., is.na(.), 0))) %>%
...
dat2<-left_join(PollenIDgenera.sum %>% pivot_wider(., names_from = "Genus", values_from = "PropReads"), 
  ...
  arrange(BeeID) %>%
#  filter(.data$BeeID %in% TwoBarcodeBees$BeeID) %>% #optional filter for BOTH BARCODE BEES
  select(BeeID, Barcode, Bombus_spp) %>%
  ungroup() 

dat2$ShannonD<-vegan::diversity(dat, index="shannon")

dat2<-dat2 %>% group_by(BeeID, Bombus_spp) %>% summarise(ShannonD=mean(ShannonD)) %>% filter(Bombus_spp=="bimaculatus"|Bombus_spp=="griseocollis"|Bombus_spp=="impatiens")

hist(dat2$ShannonD); stats::shapiro.test(dat2$ShannonD) # NOT normal, unless you exclude bees represented by only one barcode
summary(stats::aov(ShannonD ~ Bombus_spp, data = dat2)) # either way , ...
stats::kruskal.test(ShannonD ~ Bombus_spp, data = dat2) # ...shannon diversity not sig diff


ggplot(dat2, aes(x = Bombus_spp, y = ShannonD)) +
  geom_boxplot(color = "black", aes(fill = Bombus_spp)) +
#  scale_fill_manual(values = colorBlindness::Blue2DarkRed18Steps) +
  geom_label(
    data=dat2 %>% group_by(Bombus_spp) %>% summarize(n = n_distinct(BeeID)),
    aes(x = Bombus_spp, y = -0.1, label = paste("n = ", n, sep=""))) +
  labs(x = "Bombus_spp",
       y = "Shannon diversity",
       title = "") +
  theme_bw(base_size = 18)+
...
dat<- left_join(PollenIDgenera.sum1 %>% pivot_wider(., names_from = "Genus", values_from = "CLR_PropReads"), 
  dat.Queen %>% select(Date:NestSeek), by="BeeID") %>% 
  left_join(., dat.Sites, by=c("Date", "SiteID")) %>% 
  mutate(across(Prunus:Phleum, ~replace(., is.na(.), 0))) %>%
  mutate(CapTimeHM=hms::hms(hours=CapTimeH, minutes=CapTimeM, seconds=0), JDate=julian(Date)) %>%  #create variables
...
# community data standardization using raw PropReads
# dat2<-vegan::decostand(dat2,method = "hellinger") # (*not* for CLR-transformed data)
...
#best model of full dataset, untransformed proportions
# RDA_LandCover<-vegan::rda(dat2 ~ JDate + NLCD1000_Crop + Updated_y + Barcode + NLCD1000_Herbaceous + CapTimeHM, data=dat) 

# BEST ALL DATA CLR
RDA_LandCover<-vegan::rda(dat2 ~ JDate + NLCD1000_Crop + Updated_y + Barcode, data=dat) 

#best model of subset (i.e., remove samples without both barcodes) 
...
# BEST subset (i.e., remove samples without both barcodes) MODEL with CLR transformed data
# RDA_LandCover<-vegan::rda(dat2 ~ NLCD1000_Crop + JDate + Updated_y + NLCD1000_Herbaceous + NLCD1000_Developed + Barcode + NLCD1000_Forest, data=dat) 


(adjR2<-RsquareAdj(RDA_LandCover)$adj.r.squared)
anova.cca(RDA_LandCover, by="term")
summary(RDA_LandCover)
...
arrows <- arrows %>%        
  ...
  mutate(names=ifelse(names=="BarcoderbcL","rbcL",
                      ifelse(names=="JDate", "Julian date",
                             ifelse(names=="Updated_y", "Latitude", names)))) %>%
  filter(diff>0.15) # select arrows greater than x.xx units from 0,0
...
names(colors)<-unique(sites.long$Name)
ggplot() + 
    ...
    geom_point(data=sites.long, 
               aes(x=axis1, y=axis2, color=Name), 
               size=4, alpha=1) +
    scale_color_manual(values=colors) +
#    stat_ellipse(data=sites.long, aes(x=axis1, y=axis2, colour=Bombus_spp), linewidth = 1) +
    geom_point(data=species.long, aes(x=axis1, y=axis2), size=2, alpha=1, shape=1) +
    ggrepel::geom_text_repel(
      data=rbind(species.long2, arrows), 
      aes(x = axis1, y = axis2, label = labels, fontface = "italic"), 
      direction = "both",  box.padding = 0.75
      ) + #, nudge_x = c(rep(0, 10), +0.25, rep(0,5))
    geom_segment(data=arrows, aes(x = 0, xend = axis1, y = 0, yend = axis2), arrow = arrow(length = unit(0.25, "cm")), colour="black") +
    coord_fixed(ratio=4.8/10.2) +
    theme_bw(base_size = 18)+
    guides(color = F)+
#    labs(title=paste("CLR-transformed PropReads / adj-R2=",round(adjR2, digits=2), "\nLand cover differences (n = ",records," records from ",bees," bees and ", sites, " sites)", sep=""))+
...
# Misc Summary/Counts
```{r}
# do bees on common hosts have pollen samples with a large prop reads assigned to those genera?
dat<-left_join(PollenIDgenera.sum %>% pivot_wider(., names_from = "Genus", values_from = "PropReads"), 
  dat.Queen %>% select(Date:NestSeek), by="BeeID")
